...
class ConsultaCliente:
    ...
    def extrair_base_origem(self, awb: str) -> str:
        """
        Extrai a base de origem (despacho) de um AWB consultando a tela de rastreio.
        A base de origem e o Local da PRIMEIRA movimentacao valida na tabela de etapas
        (primeira linha com sigla de aeroporto de 3 letras na coluna Local).

        Args:
            awb: Numero do AWB

        Returns:
            Sigla de 3 letras da base de origem (ex: "GRU", "REC", "BSB") ou ""
        """
        try:
            # Consulta o AWB (abre modal de rastreio)
            self.browser.voltar_aba_principal()
            self.browser._fechar_modais()
            pausa(1)
            ...
            # Extrai primeira movimentacao da tabela de etapas
            base = self._extrair_primeira_base()

            # Fecha modal
            self.browser._fechar_modais()

            if base:
                logger.info(f"AWB {awb}: base de origem = {base}")
            else:
                logger.warning(f"AWB {awb}: base de origem nao identificada")

            return base

        except Exception as e:
            logger.warning(f"Erro ao extrair base de origem do AWB {awb}: {e}")
            try:
                self.browser._fechar_modais()
            except Exception:
                pass
    ...
    def _extrair_primeira_base(self) -> str:
        """
        Pega a sigla da base de origem na tabela de etapas do modal de rastreio.
        A primeira linha com Local valido (3 letras, aeroporto) e a base de despacho.
        """
        try:
            linhas = self.driver.find_elements(By.XPATH,
                "//div[@id='modalContainer']//table//tbody//tr"
            ...
            # Descobre indice da coluna "Local" pelo header
            idx_local = -1
            try:
                headers = self.driver.find_elements(By.XPATH,
                    "//div[@id='modalContainer']//table//thead//th"
            ...
            if idx_local == -1:
                idx_local = 5  # Fallback: posicao padrao

            # Percorre linhas da PRIMEIRA pra ultima (ordem cronologica)
            for linha in linhas:
                try:
                    colunas = linha.find_elements(By.TAG_NAME, "td")
                    if idx_local >= len(colunas):
                        continue

                    local = colunas[idx_local].text.strip()
                    # Valida: sigla de aeroporto (3 letras maiusculas)
    ...
    def consultar_awb_status_apenas(self, awb: str) -> ResultadoConsulta:
        """
        Consulta um AWB pela busca rapida do Nexlog e retorna APENAS o status.
        NAO consulta TADe na SEFAZ (usado na verificacao final para ser rapido).
        
        Fluxo:
        1. Digita AWB no quickSearch, clica quickTracking
        2. Abre modal "Rastreio"
        3. Le: status operacional
        4. Fecha modal — FIM (sem consultar SEFAZ)
        """
        resultado = ResultadoConsulta(awb=awb)
        ...
        try:
            ...
            # Extrai dados do modal (status operacional, local, etapas)
            self._extrair_dados_modal(resultado)

            # Classifica status (SEM verificar comentarios/termos)
            self._classificar_status(resultado)

            # Fecha modal do rastreio — FIM (nao consulta SEFAZ)
            self._fechar_modal_rastreio()
