...
class NexlogLiberar:
    ...
    def configurar_filtros(self, data_inicial: str, data_final: str):
        ...
        # O botao tem id='searchButton' (confirmado no mapeamento do HTML).
        # A falha recorrente ("Erro ao clicar Pesquisar") acontecia porque
        # um modal/backdrop da tela anterior (comentarios do CTe) ficava
        # sobreposto, interceptando o clique. Solucao: fecha modais antes
        # e usa clique via JS como fallback (JS ignora sobreposicao).
        self.browser._fechar_modais()
        pausa(0.5)

        clicou = False

        # Tentativa 1: clique normal no id exato
        try:
            ...
            logger.debug(f"Clique normal no searchButton falhou: {e}")

        # Tentativa 2: clique via JavaScript (ignora elementos sobrepostos)
        if not clicou:
            try:
                ...
                clicou = True
                logger.debug("Clicou no searchButton via JS")
            except Exception as e:
                logger.debug(f"Clique JS no searchButton falhou: {e}")

        # Tentativa 3: busca por texto
        if not clicou:
            try:
                botao = self.driver.find_element(By.XPATH, "//button[contains(.,'Pesquisar')]")
                self.driver.execute_script("arguments[0].click();", botao)
                clicou = True
                logger.debug("Clicou em Pesquisar via texto+JS")
            except Exception as e:
        ...
        if clicou:
            ...
            raise RuntimeError("Botao Pesquisar da Retencao nao encontrado/clicavel")
    ...
    def selecionar_awbs_para_liberacao(self, awbs: Set[str]) -> Dict[str, str]:
        ...
        for awb in awbs:
            try:
                ...
                pausa(2)  # Tempo para a tabela filtrar
