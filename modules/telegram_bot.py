...
class TelegramBot:
    ...
    def _processar_update(self, update: dict):
        ...
        if comando == "/status":
            ...
            self._responder_vencimento(text)
        else:
    ...
    def _responder_ajuda(self):
        ...
        msg = (
            ...
            "/vencimento <awb> - Consultar data de vencimento\n"
    ...
    def _responder_vencimento(self, texto_completo: str):
        """Responde ao /vencimento <awb> — consulta vencimento de AWB."""
        partes = texto_completo.strip().split()
        if len(partes) < 2:
            self._enviar_mensagem(
                "Uso: /vencimento <numero_awb>\n"
                "Exemplo: /vencimento 12712345678"
            )
            return
        ...
        if not awb.isdigit() or len(awb) < 10:
            self._enviar_mensagem(f"AWB invalido: {awb}")
            return

        self._enviar_mensagem(f"Consultando vencimento AWB {awb}...")

        # Executa em thread separada (abre browser headless)
        import threading
        threading.Thread(
            target=self._executar_vencimento, args=(awb,), daemon=True
        ).start()

    def _executar_vencimento(self, awb: str):
        """Executa consulta de vencimento em thread separada."""
        try:
            ...
            resultado = dom.consultar_vencimento(awb)

            browser.fechar()

            if resultado.erro:
                self._enviar_mensagem(f"Erro: {resultado.erro}")
            else:
                self._enviar_mensagem(
                    f"AWB {awb}\n"
                    f"Vencimento: {resultado.data_vencimento}\n"
                    f"Status: {resultado.status_carga}"
                )
        except Exception as e:
            self._enviar_mensagem(f"Erro na consulta: {str(e)[:100]}")
